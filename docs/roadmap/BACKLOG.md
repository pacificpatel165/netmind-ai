...
| 21 | Local model benchmark (Ollama, size/latency on this machine) | #7 diagnosis assistant | `ollama pull llama3.2:3b` + a test prompt confirmed working (2026-09-11) — the model responds correctly. Latency/RAM headroom alongside the other 7 lab containers not yet measured, and the 8B-class model hasn't been tried for comparison | Time a representative prompt (retrieved-doc chunk + a metrics summary + a question) against both 3B and, if the machine can take it, an 8B model, then pick one deliberately rather than by default | 2026-09-11, log (19) |
