...
| 22 | Ollama placement (host vs. containerized) and local-vs-online provider abstraction | #7 diagnosis assistant | Model decision (8B) landed 2026-09-11, but whether Ollama moves into the topology as its own node (consistent with every other component) or stays host-installed, and whether Groq/Gemini get wired in behind a shared provider interface, are both still open — see `docs/setup/07-diagnosis-assistant.md` | Placement decided once GPU-passthrough plans (if any) are clearer; provider abstraction built once local-only proves insufficient for some real question, not speculatively | 2026-09-11, log (22) |
