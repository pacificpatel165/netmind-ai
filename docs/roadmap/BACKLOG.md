...
| 6 | Pin container image tags | #2, #3 | `gnmic` and `prom/prometheus` both currently track `:latest` in the topology file, written without a chance to confirm the resolved version first | Both pinned to the exact tag confirmed working, so an upstream release can't silently change lab behavior between sessions | 2026-09-10, log (6, 9) |
...
| 10 | gnmic → Prometheus metric names unverified | #3 metrics store | `metric-prefix`/`append-subscription-name` config was written without seeing real scrape output | Confirm actual metric names once `http://localhost:9090` is checked; correct `metrics/README.md`'s example query if the real names differ | 2026-09-10, log (9) |
| 11 | Components #4–#11 | Grafana, anomaly detection, retrieval index, diagnosis assistant, remediation proposal, security gate, config-push executor, IaC/environment provisioning | Not started — next in build order after #3 stage 1 closes out | Full descriptions live in `PROGRESS_LOG.md`'s Component map; not duplicated here on purpose, so there's one place to check for "what's actually left" | ongoing |

## Resolved

*(empty — move a row here from the table above once it's actually
addressed, with the date and the `PROGRESS_LOG.md` entry that closed
it out)*
