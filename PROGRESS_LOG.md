...
**Real bug hit before the playbook even started, not a playbook issue:** `pip install --user ansible-core --break-system-packages` succeeded, but every attempt to run it — `ansible-playbook ...` (twice, different `git_email` values) and even bare `ansible` — failed identically: `ERROR: Ansible could not initialize the preferred locale: unsupported locale setting`. Root-caused via WebSearch/WebFetch (a known Ansible/locale GitHub issue plus a corroborating blog post) rather than guessed: the `wsl-containerlab` image has no UTF-8 locale generated at all. Confirmed directly against this exact machine before proposing a fix: `locale -a` returned only `C`, `C.utf8`, `POSIX` (three "Cannot set LC_*" warnings alongside it) — matching the diagnosis exactly, not just plausible in general. Fixed with glibc's built-in `C.UTF-8` (no `locale-gen` needed, unlike `en_US.UTF-8` which usually isn't present on a minimal image): `export LC_ALL=C.UTF-8`, persisted to both `~/.bashrc` and `~/.profile` (the same double-persist pattern already used for the `~/.local/bin` PATH fix, entry 57, since interactive-only `.bashrc` sourcing has already bitten this project once). `ansible --version` then ran clean.

**Real play run, full output pasted back:**
...
The one `changed` task was `Ensure ~/.kube exists` — tightening that directory to `0700`, a genuine correct fix (it existed already but wasn't mode-restricted), not a bug surfaced. Every other task reported `ok` (Docker/containerlab/gnmic/rsync/Cilium CLI all already present from earlier sessions — the check-first guards worked exactly as designed, no re-install, no error) or `skipping` (SSH key already existed, kubeconfig already wired).

**Honest caveat, not glossed over:** because this machine was already fully bootstrapped before this playbook existed, this run only exercised the "already installed, skip" code paths. The actual first-time-install branches — the Docker CE/containerlab combined script, the gnmic install script, and the Cilium CLI download-checksum-extract block — never fired, since every one of their `when:` guards evaluated false. Those remain genuinely unverified until a truly blank machine runs this playbook, which is exactly what item 41 (from-scratch rebuild test) exists to do. Not deferring dishonestly — stating plainly that "closed" here means "idempotency/skip-path proven," not "every code path exercised."

**Cosmetic bug found in the real output, fixed:** the final summary task printed `"containerlab: OK — "` (blank) instead of a real version string — `clab version`'s first `stdout_lines` entry is a blank ASCII-banner line, not text, so `item.stdout_lines[0]` picked up nothing. Fixed by selecting the first non-empty line instead (`(item.stdout_lines | select('trim') | list | first)`), re-validated (`python3 -c "import yaml; ..."` → valid, `ansible-playbook --syntax-check` → clean).

**Docs updated to match the real result, not left stale:** `provisioning/ansible/README.md` — added a new "0. Fix the locale first" step to "One-time setup" documenting the bug and fix exactly as found; replaced the old "Not yet independently verified... not yet run live" closing paragraph in "Verifying it" with the real play recap, the `changed=1` explanation, and the same first-install-paths-unexercised caveat above.

**Item 11 — closed for the idempotency/skip-path guarantee, 2026-09-19.** Full first-time-install verification (Docker/containerlab/gnmic/Cilium CLI actually installing from nothing) explicitly deferred to item 41, not silently assumed to work because the skip-path did.

### 2026-09-19 (66) — Item 11 started: Ansible host-bootstrap playbook written, not yet run live

**Focus:** first real work on component #11, after a deliberate scoping conversation (not defaulted to the original "Terraform/Ansible for #1/#3/#4" wording literally). Walked through why re-managing containerlab's own containers in Terraform would duplicate working declarative orchestration for no real gain, versus the actual, concrete pain point: `docs/setup/01-network-lab-environment.md`'s host-bootstrap steps (Docker CE + containerlab, gnmic, rsync, SSH key, and — found missing from the doc entirely just yesterday — kubeconfig wiring and Cilium CLI install) only ever existed as commands a person types by hand. User asked to understand IaC concepts properly before deciding scope — built and published an explainer Artifact ("Infrastructure as Code") covering declarative-provisioning vs. configuration-management, what's already IaC in this project (containerlab, the AWS Terraform exercise) vs. what isn't (the manual bootstrap), a before/after pipeline diagram, and a concrete definition of done. User confirmed the recommended scope (Ansible host bootstrap) after reviewing it.

**Built:** `provisioning/ansible/bootstrap-lab-host.yml` — one playbook, 18 tasks, every install task guarded by a check-first task (`stat`/`command` + `when:`) so re-running it on an already-bootstrapped machine reports `ok` everywhere rather than erroring or repeating work. Covers §5 (Docker CE + containerlab, via containerlab's own official combined install script — no clean native-module equivalent since it installs two things together), §8 (gnmic), §9 (rsync, via the real `ansible.builtin.apt` module), §10 (ed25519 SSH key generation — key only, GitHub registration stays a manual step needing human login), and the Kubernetes section's kubeconfig wiring (skipped with a clear message if k3s isn't installed yet, not a hard failure) and Cilium CLI install (checksum-verified, same install script logic as `docs/setup/01-network-lab-environment.md`'s own Cilium CLI section, ported into Ansible tasks). Ends with a summary task that re-checks every tool's real state independently rather than trusting the install tasks' own "changed" status.

**Deliberately out of scope, and documented as such in the playbook's own header and the README:** sections 2-4 (Windows-side: `wsl --update`, installing the distro itself, Docker Desktop's WSL-integration toggle — none reachable from inside the distro Ansible runs in), registering the SSH key on GitHub (needs a human's GitHub login), and installing k3s/Cilium themselves (both already effectively single idempotent commands — the actual gap was everything *around* them, not those two commands).

**Verified what could be verified without the real machine, honest about what couldn't:** YAML parsed cleanly (`python3 -c "import yaml; ..."`), and `ansible-playbook --syntax-check` (installed ansible-core into this session's own sandbox to run it for real, not simulated) passed clean. A full `--check` dry-run hit an environment mismatch — this sandbox is Fedora-based, no `apt`/`python3-apt`, while the real target is Debian — so the `apt` module's check-mode requirement failed immediately; **not a playbook bug, an unavoidable limitation of testing a Debian-targeted playbook on a non-Debian sandbox.** Told the user plainly rather than presenting the syntax-check pass as more confidence than it earns — genuine live execution on the real WSL2 distro is still outstanding.

**Also chose `pip install --user ansible-core --break-system-packages` over apt for installing Ansible itself** (documented in the new `provisioning/ansible/README.md`) — couldn't get a clean, current confirmation of Debian bookworm's apt-repo `ansible` package freshness through this session's proxy, and the pip pattern is already proven working on this exact distro from the LocalStack/Terraform exercise (including its own PATH-fix for `~/.local/bin`, reused here rather than re-derived).

**Not yet done — the actual next step, not deferred:** run `ansible-playbook bootstrap-lab-host.yml` for real on the `Containerlab` distro and paste back the genuine output, same as every other live-system claim in this project. Item 11 stays open until that happens.
...
**Enabling BGP Control Plane broke the whole Cilium agent, not just BGP — caught and fixed before touching any BGP config:** `cilium upgrade --set bgpControlPlane.enabled=true` flipped the ConfigMap flag correctly (confirmed via `kubectl get configmap cilium-config -o yaml`) but warned pods needed a manual restart. Forcing `kubectl rollout restart ds/cilium` left the agent stuck `0/1 Running`, repeatedly restarting, **Startup probe failing on its own healthz port** (`dial tcp 127.0.0.1:9879: connect: connection refused`) — a real, cluster-wide-impact problem, not a BGP-specific one, since a broken CNI agent would have jeopardized the already-working proof-app and CoreDNS fix from entry 62 too.

**Root cause, found by checking the Cilium Operator (not yet looked at) rather than guessing from the agent's own logs alone:** the agent's logs showed it endlessly waiting for "Cilium Operator to register CRDs" for the 5 new BGP resource types. Checking `kubectl get crds | grep bgp` confirmed directly — **none of the 5 BGP CRDs existed in the cluster at all.** `cilium upgrade` toggled the feature flag but never pushed the new CRD manifests; the operator's own boot log only auto-creates its fixed legacy CRD list (identities, endpoints, node configs, etc.), never touching the new BGP ones. Not a version-skew bug (briefly suspected from a `PartialObjectMetadata` decode warning in the agent's logs, which turned out to be unrelated noise from the same failed watch loop) — verified the CRD manifests for `cilium-cli`'s own reported running version (`v1.20.1`, confirmed via `cilium version` before applying anything) exist at that exact tag in Cilium's GitHub repo, and applied all 5 directly via `kubectl apply -f https://raw.githubusercontent.com/cilium/cilium/v1.20.1/.../crds/v2/<name>.yaml`. Agent recovered on its own within seconds — `1/1 Running`, no further restarts.

**Wrote and applied `k8s/cilium-bgp-peering.yaml`** (`CiliumBGPClusterConfig` + `CiliumBGPPeerConfig` + `CiliumBGPAdvertisement`, schema verified live against docs.cilium.io before writing — not guessed) — applied cleanly, but `cilium bgp peers` came back completely empty (not even an idle/active row).

**Second real bug, same class as the first — a stale operator, not a config mistake:** `kubectl get ciliumbgpnodeconfig` (the resource the operator is supposed to generate per-node from the `ClusterConfig`) showed nothing, and the `ClusterConfig` itself had zero `Events`. Root cause: `cilium-operator`'s pod was 25h old, last restarted 23h ago — it had established its `CiliumBGPClusterConfig`/`CiliumBGPNodeConfig` watch loops (visible in its own earlier boot log) long before the BGP CRDs existed, and never recovered those watches on its own once they were created. Fixed: `kubectl rollout restart deployment cilium-operator`. Within seconds of the new operator pod coming up: `ciliumbgpnodeconfig` appeared, `cilium bgp peers` showed `prashant-patel` peering to `192.168.99.2`, **Session State: established**, advertising `10.0.0.0/24` (the real Pod CIDR).

**Verified bidirectionally, not just from Cilium's side:** `srl1`'s own `show network-instance default protocols bgp neighbor` flipped from `active` to `established`, `AFI/SAFI [1/0/0]` (1 route received). But `show network-instance default protocols bgp routes ipv4 summary` showed a **third real bug**: `10.0.0.0/24` was received but sitting with a blank status column (not `u*>` like the directly-connected `/30`), and the summary line read `2 received BGP routes: 1 used, 1 valid` — the Pod CIDR route was present but not actually usable. Root cause, confirmed against Nokia's own SR Linux BGP docs before touching config: **SR Linux applies a default-reject import policy on eBGP sessions** — routes are received onto the wire but discarded from the RIB unless an explicit import policy (or the global override) says otherwise. Fixed on `srl1`: `set / network-instance default protocols bgp ebgp-default-policy import-reject-all false`. Re-checked immediately: both routes now `u*>`, `2 received BGP routes: 2 used, 2 valid, 0 stale`.

**Real end state, verified from both sides, not assumed:** Cilium (`prashant-patel`, ASN 65002) ↔ `srl1` (ASN 65001) BGP session `established` on both ends; Pod CIDR `10.0.0.0/24` advertised, received, and validly installed in `srl1`'s route table via the directly-connected `192.168.99.0/30` underlay link wired in entry 63.

**Pattern worth naming explicitly:** all three bugs this session were the *same shape* — a component (operator, agent, SR Linux's RIB) had already made a decision (start its watch loops; probe on port 9879; reject-by-default) before the thing it depended on existed or was told otherwise, and none of them self-healed once the dependency showed up. Every one was root-caused by checking the specific component that owned the stuck state directly (Operator logs, not just agent logs; `kubectl get crds`, not assuming from agent output; SR Linix's own BGP docs, not assuming the routes table would just populate) rather than guessing from one layer removed.

**Known gap, unchanged from entry 63, not solved here:** none of this session's `srl1` config (interface addressing, network-instance binding, BGP config, and now the `ebgp-default-policy` change) has a `startup-config` — still wiped on the next `clab destroy`/`clab deploy`. Still tracked as `BACKLOG.md` item 38, deliberately not folded into this entry — same reasoning as entry 63, avoid compounding untested surface area right after a multi-round live debugging session. The exact proven command sequence (interface + BGP + policy) is fully documented in `docs/setup/01-network-lab-environment.md` for fast reapplication.

**Item 4 — closed, 2026-09-18.** Every piece live-verified: k3s + Cilium installed (entry 53), real workload deployed with a real CoreDNS/WSL2 bug found and fixed (entry 62), `e1-2` underlay link wired and addressed (entry 63), and now BGP peering fully established with the Pod CIDR genuinely reachable from `srl1`'s own route table. Next up on the working-order checklist: item 11 (IaC/provisioning — Terraform/Ansible for components #1/#3/#4), per the user's explicit instruction to finish item 4 first.

### 2026-09-18 (63) — Item 4, underlay link live: `e1-2` addressed and pinging both directions after three real SR Linux gotchas

**Focus:** item 4's second remaining piece, first sub-step — get the reserved `e1-2` interface wired, addressed, and reachable between `srl1` and the WSL2 host (where k3s/Cilium run natively), before touching BGP at all.

**Topology change:** `lab/topologies/netmind-2node.clab.yml` — added `- endpoints: ["srl1:e1-2", "host:srl1_e1-2"]`, using containerlab's reserved `host` node name (verified against containerlab's own docs — not guessed): creates a veth pair with one end inside `srl1`'s namespace as `e1-2`, the other landing directly in the WSL2 host's root namespace as `srl1_e1-2`. Full destroy + redeploy applied cleanly; both ends confirmed present (`show interface ethernet-1/2` on `srl1`, `ip addr show srl1_e1-2` on the host) before any addressing was attempted.

**Addressing hit three real, sequential SR Linux data-model gotchas — each root-caused from live `show`/`info` output, none guessed:**

1. **`ping ... network-instance default` initially failed** with "Must be an existing network namespace." Cause: SR Linux keeps a subinterface's addressing (under `interface`) and its actual routing participation (under `network-instance`) as two genuinely separate things — a subinterface with an IP isn't automatically part of any network-instance. Fixed: `set / network-instance default interface ethernet-1/2.0`.
2. **After that, `ping` returned a different, more specific error: "Network is unreachable."** `show interface ethernet-1/2 detail` showed the real cause directly: `Subinterface: ethernet-1/2.0` → `Oper state: down`, `Down reason: no-ip-config` — despite an IPv4 address clearly being configured. Tried bouncing the subinterface's own admin-state (disable/enable) first, based on the reasonable-sounding theory that the down-reason was just stale; that did **not** fix it (`Last change` timestamp didn't reset), which itself was useful negative evidence — ruled out "stale state" as the cause.
3. **Real cause, confirmed against Nokia's own docs before typing more config:** SR Linux has a *second, independent* admin-state switch specifically for the IPv4 address-family under a subinterface, separate from the subinterface's own admin-state — `interface ethernet-1/2 subinterface 0 ipv4 admin-state enable`. Never having enabled it is exactly what "no-ip-config" meant, and it's why the first bounce (of the wrong switch) did nothing. Setting this one flipped `Oper state` to `up` immediately (`Last change: 8s ago`, down-reason cleared).

**Real proof, both directions:** `srl1 → host`, 13/13 packets, 0% loss (`ping 192.168.99.1 network-instance default`); `host → srl1`, 3/3 packets, 0% loss (`ping -c 3 192.168.99.2`). The link is genuinely live at L3 now, not just "interfaces show up."

**Known gap, not yet solved — flagged rather than silently left:** `srl1` has no `startup-config` in the topology (deliberate, matches every other node here — nothing has persistent config yet), so this entire manual CLI sequence will be wiped by the next `clab destroy`/`clab deploy` cycle, same as everything else on these nodes. Documented the full, exact, proven command sequence in `docs/setup/01-network-lab-environment.md` so it can be reapplied quickly rather than re-derived from scratch — a `startup-config` JSON or an idempotent script to automate re-applying it is a reasonable future improvement, logged as a new `BACKLOG.md` entry rather than solved now, to avoid introducing more untested surface area on top of what was already a three-round debugging session.

**Not yet done:** the actual point of this link — Cilium peering BGP with `srl1` over it and advertising the pod CIDR. That's next.
